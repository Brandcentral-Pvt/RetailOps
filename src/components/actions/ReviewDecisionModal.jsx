  ...
  if (!isAuthorized) {
    return (
      <Modal open={isOpen} onCancel={onClose} centered width={400} closable={false}
        footer={<Button type="primary" block onClick={onClose} style={{ borderRadius: "var(--radius-md)", fontWeight: 600, fontSize: 'var(--font-size-xs)' }}>Close</Button>}
        title={<span style={{ fontSize: 'var(--font-size-base)', fontWeight: 600 }}>Not Authorized</span>}>
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          <LockOutlined style={{ fontSize: 32, color: '#C62828', marginBottom: 12 }} />
          <div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 600, color: '#18181b', marginBottom: 4 }}>Only {formatUserName(task.reviewer)} can review</div>
        </div>
      </Modal>
    );
  }
    ...
    <Modal open={isOpen} onCancel={onClose} footer={null} width={600} centered closable={!submitting}
      ...
      title={
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: "var(--radius-md)", background: '#f5f3ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <EyeOutlined style={{ fontSize: 'var(--font-size-base)', color: '#9C27B0' }} />
          </div>
          <div>
            <div style={{ fontSize: 'var(--font-size-base)', fontWeight: 600, color: '#18181b' }}>Review Submission</div>
            <div style={{ fontSize: 'var(--font-size-xs)', color: '#a1a1aa', marginTop: 1 }}>{task.title || task.action || task.name}</div>
          </div>
        </div>
      }
    >
      {/* Worker's Submission */}
      <div style={{ background: '#f8fafc', border: '1px solid #e4e4e7', borderRadius: "var(--radius-lg)", padding: 16, marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 28, height: 28, borderRadius: "var(--radius-md)", background: '#18181b', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 'var(--font-size-xs)', fontWeight: 600 }}>
              {assignee ? (assignee.firstName || assignee.name || 'U').charAt(0).toUpperCase() : 'U'}
            </div>
            <div>
              <div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 600, color: '#18181b' }}>{formatUserName(assignee)}</div>
              <div style={{ fontSize: 10, color: '#a1a1aa' }}>Assignee</div>
            </div>
          </div>
          <Tag style={{ fontSize: 9, borderRadius: "var(--radius-sm)", background: '#f5f3ff', color: '#9C27B0', border: '1px solid #ddd6fe', fontWeight: 600, margin: 0 }}>SUBMITTED</Tag>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 600, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>Work Completed</div>
            <p style={{ fontSize: 'var(--font-size-sm)', color: '#3f3f46', margin: 0, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
          ...
          {submission.outcome && (
            <div>
              <div style={{ fontSize: 10, fontWeight: 600, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>Result</div>
              <p style={{ fontSize: 'var(--font-size-sm)', color: '#3f3f46', margin: 0 }}>{submission.outcome}</p>
            </div>
          )}

          {(submission.before || submission.after) && (
            <div style={{ display: 'flex', gap: 8 }}>
              {submission.before && <Tag style={{ fontSize: 10, borderRadius: "var(--radius-sm)", background: '#fef2f2', color: '#C62828', border: '1px solid #fecaca', fontWeight: 600, margin: 0 }}>Before: {submission.before}</Tag>}
              {submission.after && <Tag style={{ fontSize: 10, borderRadius: "var(--radius-sm)", background: '#ecfdf5', color: '#2E7D32', border: '1px solid #a7f3d0', fontWeight: 600, margin: 0 }}>After: {submission.after}</Tag>}
            </div>
          )}

          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #e4e4e7', paddingTop: 10, fontSize: 'var(--font-size-xs)', color: '#71717a' }}>
          ...
          {submission.reviewerNotes && (
            <div style={{ background: '#fff', border: '1px solid #e4e4e7', borderRadius: "var(--radius-md)", padding: 10, fontSize: 'var(--font-size-xs)', color: '#52525b' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 10, fontWeight: 600, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Your Decision</div>
        <div style={{ display: 'flex', gap: 10 }}>
          <div onClick={() => setDecision('APPROVE')} style={{
            ...
            <CheckCircleOutlined style={{ fontSize: 'var(--font-size-xl)', color: '#2E7D32' }} />
            <div>
              <div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 600, color: '#065f46' }}>Approve</div>
          ...
          <div onClick={() => setDecision('REJECT')} style={{
            ...
            <CloseCircleOutlined style={{ fontSize: 'var(--font-size-xl)', color: '#C62828' }} />
            <div>
              <div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 600, color: '#D32F2F' }}>Reject</div>
      ...
      {decision && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 'var(--font-size-xs)', fontWeight: 600, color: '#52525b', marginBottom: 4 }}>
          ...
          {decision === 'REJECT' && (
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 'var(--font-size-xs)', fontWeight: 600, color: '#52525b', marginBottom: 4 }}>New Deadline</div>
                <DatePicker style={{ width: '100%', borderRadius: "var(--radius-md)" }} size="small"
                  disabledDate={d => d && d.isBefore(dayjs().startOf('day'))}
                  value={newDueDate} onChange={setNewDueDate} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 'var(--font-size-xs)', fontWeight: 600, color: '#52525b', marginBottom: 4 }}>Reason</div>
                <Select size="small" placeholder="Select" value={rejectionCategory}
                  onChange={setRejectionCategory} allowClear style={{ width: '100%', borderRadius: "var(--radius-md)" }}
                  options={[
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, borderTop: '1px solid #f4f4f5', paddingTop: 12 }}>
        <Button onClick={onClose} disabled={submitting} style={{ borderRadius: "var(--radius-md)", fontSize: 'var(--font-size-xs)' }}>Cancel</Button>
